...
# Upgrade `when` to a MagicMock to allow call assertions
mock_when = MagicMock()
def side_effect_when(condition, value):
    m = MagicMock()
    m.otherwise = MagicMock()
    return m
mock_when.side_effect = side_effect_when
...
class TestRelatorioProdutosMensal(unittest.TestCase):
    def setUp(self):
        ...
        mock_when.reset_mock() # Reset calls before each test
