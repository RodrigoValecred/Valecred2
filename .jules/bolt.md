...
## 2026-05-08 - Flatten PySpark Catalyst StackOverflow with .withColumnRenamed() Chains (Re-applied in PDD)
**Learning:** Continuing from previous learnings, deeply chained `.withColumnRenamed()` operations (like those found in `NB_Silver_Carteira_PDD.Notebook`) create excessive `Project` nodes in the Catalyst logical plan, severely degrading compilation performance and risking `StackOverflowError`.
**Action:** Replaced chained `.withColumnRenamed()` calls with list comprehensions mapping existing columns through a dictionary and applying them via a single `.toDF(*new_columns)` operation. This collapses the logical plan modifications into a single optimized projection node.
