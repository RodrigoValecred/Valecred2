...
from pyspark.sql.functions import (
    ...
    datediff, sum, min, count, round, floor, least, current_date, split, pow, xxhash64, unix_timestamp, ceil, abs, hour, month, weekofyear, last_day, months_between, trunc
)
...
# Mapeamento de Status da Esteira para nomes limpos
...
def create_cad_geral_enriquecido(df_geral_pf_pj_limpa, df_enderecos_limpa, df_emails_agg, df_telefones_agg):
    """
    Creates the enriched general registration DataFrame.
    """
    print("Criando DataFrame intermediário: Cadastro Geral Enriquecido...")
    return df_geral_pf_pj_limpa \
...
def create_gerentes_enriched(df_gerentes, df_usuarios, df_geral_pf_pj_limpa, df_plataformas):
    """
    Creates the enriched managers DataFrame.
    """
    ...
    return df_gerentes_plat.select(
        col("g.cod_broker"),
        col("g.taxa_comissao"),
...
def create_operacoes_enriquecida(
    ...
    """
    Creates the enriched operations DataFrame.
    """
    print("Criando DataFrame intermediário: Operações Enriquecidas...")

    # PRE-CALCULO: Data Primeira Operação por Cliente
    ...
    # PRE-CALCULO: Taxa Cadastro do Cliente
    ...
    return df_ops_enrich_step1.withColumn(
        "operacao_informal",
        when(
...
# Internal helpers for create_fato_operacoes
...
def create_fato_operacoes(df_operacoes_enriquecida, df_dim_calendario, df_dim_produto):
    """
    Orchestrates the creation of fato_operacoes.
    """
...
def create_fato_baixas(
    ...
    return df_enriquecido_baixas.select(
...
def create_fato_titulos(
    ...
    """
    Creates the fato_titulos DataFrame.
    """
...
def create_fato_prorrogacoes(spark, df_titulos_limpa, df_operacoes_limpa):
    """
    Creates the fato_prorrogacoes_de_titulos DataFrame.
    """
...
def create_fato_operacoes_prorrogacao(spark, df_operacoes_limpa, df_titulos_limpa):
    """
    Creates the fato_operacoes_prorrogacao DataFrame.
    """
    ...
    df_boletos_select = df_boletos_titulos.select(
        col("cod_operacao"), col("cod_titulo"), col("n_doc"), col("cpf_cnpj_sacado"),
        col("cpf_cnpj_cedente"), col("valor"), col("amortizacoes"), col("liquidacao")
    )

    df_joined_pr = df_ops_pr_clean.join(df_boletos_select, "cod_operacao", "left")
    return df_joined_pr.drop("status_analise", "status_aceite", "tto")

def create_fato_recompra(spark, df_operacoes_limpa, df_titulos_limpa):
    """
    Creates the fato_operacoes_recompra DataFrame.
    """
    df_operacoes_source = df_operacoes_limpa
    ...
    if "chave_base_operacao" in df_joined_rc_clean.columns:
        return df_joined_rc_clean.withColumnRenamed("chave_base_operacao", "chave_base_operacao_recompra")
    else:
        return df_joined_rc_clean.withColumn("chave_base_operacao_recompra", concat(lit("40-"), col("cod_operacao")))
...
def create_fato_limites_credito(
    ...
    # Preparar Limites Manuais
    df_limites_ep_prep = df_limites_extra_plus.withColumn("cnpj_clean", regexp_replace(col("cnpj"), "[^0-9]", ""))
    df_limites_ep_clientes = df_limites_ep_prep.join(df_clientes_staging.select(col("cpf_cnpj").alias("cnpj_clean"), "cod_cliente"), "cnpj_clean", "inner")
    df_limites_ep_grupos = df_limites_ep_clientes.join(df_grupos_prep, "cod_cliente", "inner")
    ...
    # Tratamento Clientes Individuais
    ...
    return df_grupo_final.unionByName(df_cliente_final, allowMissingColumns=True) \
...
def create_dim_clientes(
    ...
    """
    Creates dim_clientes, analise_score_clientes, and analise_prazos_esteira.
    Returns (df_dim_clientes, df_score_clientes, df_analise_prazos)
    """
    ...
    df_perdas_agg = df_titulos_cliente.filter(col("motivo") == "PR") \
        .groupBy("cod_cliente").agg((sum("valor") - sum("amortizacoes")).alias("perdas"))
    ...
    if "CODCLIENTE" in df_esteira.columns: df_esteira = df_esteira.withColumnRenamed("CODCLIENTE", "cod_cliente")
    if "STATUS_DO_CLIENTE" in df_esteira.columns: df_esteira = df_esteira.withColumnRenamed("STATUS_DO_CLIENTE", "status_do_cliente")
    if "DATALOG" in df_esteira.columns: df_esteira = df_esteira.withColumnRenamed("DATALOG", "datalog")
    ...
    df_esteira_latest = df_esteira.withColumn("rn", row_number().over(w_latest)).filter(col("rn") == 1) \
        .select(col("cod_cliente").alias("cod_cliente_latest"), col("status_do_cliente").alias("status_do_cliente"), col("macroprocesso").alias("MACROPROCESSO"), col("fase").alias("FASE"), col("datalog").alias("data_status"))
    ...
    df_join_1 = join_cliente_dimensions(
        df_base, df_cad_geral_enriquecido, df_metrics_ops_final, df_metrics_titulos_final,
        df_esteira_pivot_prep, df_esteira_min_prep, df_esteira_latest, df_limites_agg,
        df_grupos_prep, df_limites_grupo_dedup, df_risco_grupo_agg, df_info_gestor,
        df_client_rate_gold, df_status_cad_prep
    )

    df_funnel = df_join_1 \
        .withColumn("data_primeira_proposta_comercial", least(col("min_proposta"), col("min_revisao"), col("min_dir_comercial"))) \
        .withColumn("data_primeira_proposta_credito", when(col("min_credito") >= col("data_primeira_proposta_comercial"), col("min_credito"))) \
        .withColumn("data_primeira_proposta_formalizacao", when(col("min_checklist") >= col("data_primeira_proposta_credito"), col("min_checklist"))) \
        .withColumn("data_primeira_proposta_concluida", when(col("min_concluido") >= col("data_primeira_proposta_formalizacao"), col("min_concluido")))
    ...
    df_final = calculate_funnel_dates(df_funnel) \
        ...
        .withColumn("disponivel_comissaria", greatest(least(col("limite_disponivel"), col("limite_comissaria_contrato") - col("risco_subtotal_comissaria")), lit(0))) \
        ...
        .withColumn("status_atividade", when(col("dias_sem_operar") > 120, "INATIVO").when(col("data_ultima_operacao").isNull(), "NUNCA OPEROU").otherwise("ATIVO")) \
        .withColumn("status_limite", when((col("limite").isNull()) | (col("limite") == 0), "SEM LIMITE").when(col("vencimento_limite") < today_date, "LIMITE VENCIDO").when(col("risco") == 0, "LIMITE INATIVO").when(col("risco") > col("limite"), "LIMITE EXCEDIDO").otherwise("LIMITE DISPONIVEL")) \
        .withColumn("percentual_cm", col("limite_comissaria_contrato") / col("limite")) \
        .withColumn("falta_checar", greatest((coalesce(col("percentual_exigido"), lit(0.5)) * (col("risco_exceto_comissaria") + col("risco_grupo") - col("risco_subtotal_comissaria"))) - coalesce(col("confirmado_positivo"), lit(0)) - coalesce(col("confirmado_atencao"), lit(0)), lit(0))) \
        .withColumn("data_primeira_operacao_apos_aprovacao", when(col("data_primeira_operacao") >= col("data_aprovacao"), col("data_primeira_operacao"))) \
        ...
        .withColumn("tipo_proposta", when(col("dias_sem_operar") > 120, "REATIVAÇÃO").when(col("data_ultima_operacao").isNull(), "PROSPECÇÃO").when(col("idade_cliente_em_dias") > 90, "RENOVAÇÃO").otherwise("PROSPECÇÃO")) \
        .withColumn("pais", lit("Brasil")) \
        .withColumn("primeiro_nome_gerente", split(col("gestor_da_plataforma"), " ")[0]) \
        .withColumn("id_limite_credito", when(col("grupo_economico").isNotNull(), concat(lit("G-"), upper(trim(col("grupo_economico"))))).otherwise(concat(lit("C-"), col("cod_cliente")))) \
        .withColumn("status_operando_vencido", when((col("data_ultima_operacao") > col("vencimento_limite")) & (col("vencimento_limite").isNotNull()) & (date_add(col("data_ultima_operacao"), 120) >= today_date), "OPERANDO VENCIDO").otherwise("OPERANDO NORMAL")) \
        .withColumn("taxa_minima_exigida", (when(col("faixa_pdd") == 0.05, 0.0025).when(col("faixa_pdd") == 0.7, 0.0075).otherwise(0.0050)) + 0.0150 + 0.0010) \
        .withColumn("pendencias_desc", concat_ws(", ", when(col("status_atividade") == "INATIVO", "Cliente inativo"), when(col("falta_checar") > 0, "Confirmação desenquadrada"), when(col("vencimento_limite") < today_date, "Limite vencido"), when(col("limite_maximo_disponivel") <= 0, "Sem limite disponível"), when(col("problemas_checagem") > 0, "Problemas de checagem"), when(col("inadimplencia") > 0, "Títulos vencidos"))) \
        .withColumn("qtd_pendencias", (when(col("status_atividade") == "INATIVO", 1).otherwise(0) + when(col("falta_checar") > 0, 1).otherwise(0) + when(col("vencimento_limite") < today_date, 1).otherwise(0) + when(col("limite_maximo_disponivel") <= 0, 1).otherwise(0) + when(col("problemas_checagem") > 0, 1).otherwise(0) + when(col("inadimplencia") > 0, 1).otherwise(0))) \
        ...
        .withColumn("penalidade_inativo", when(col("dias_sem_operar") > 120, 3).when(col("dias_sem_operar") > 60, 2).when(col("dias_sem_operar") > 30, 1).otherwise(0)) \
        .withColumn("penalidade_perdas", when(col("perdas") > 0, 10).otherwise(0)) \
        .withColumn("perc_vencidos_risco", col("vencidos") / col("risco")) \
        .withColumn("penalidade_inadimplencia", when(col("perc_vencidos_risco") > 0.50, 4).when(col("perc_vencidos_risco") > 0.25, 3).when(col("perc_vencidos_risco") > 0.10, 2).when(col("perc_vencidos_risco") > 0.05, 1).otherwise(0)) \
        .withColumn("perc_risco_renegociacao", col("risco_renegociacao") / col("risco")) \
        .withColumn("penalidade_renegociacao", when(col("perc_risco_renegociacao") == 1, 3).when(col("perc_risco_renegociacao") > 0.4, 2).when(col("perc_risco_renegociacao") > 0.01, 1).otherwise(0)) \
        .withColumn("qualidade_cliente", when(col("penalidade_perdas") > 0, 0).otherwise(round(lit(10) - (col("penalidade_inadimplencia") + col("penalidade_renegociacao") + col("penalidade_clean") + col("penalidade_inativo")), 0)))

    # Separar Tabelas
    cols_score = ["perc_risco_clean", "penalidade_clean", "penalidade_inativo", "penalidade_perdas", "perc_vencidos_risco", "penalidade_inadimplencia", "perc_risco_renegociacao", "penalidade_renegociacao", "qualidade_cliente"]
    df_score_clientes = df_final.select("cod_cliente", *cols_score)
    df_final = df_final.drop(*cols_score)

    cols_esteira_prazos = [
        "pivot_checklist", "pivot_assinatura", "pivot_comite", "pivot_concluido", "pivot_bizagi", "pivot_renovacao", "pivot_reserva", "pivot_start", "pivot_credito", "pivot_proposta", "pivot_revisao_comercial", "pivot_dir_comercial",
        "min_proposta", "min_revisao", "min_dir_comercial", "min_credito", "min_checklist", "min_concluido",
        "data_primeira_proposta_comercial", "data_primeira_proposta_credito", "data_primeira_proposta_formalizacao", "data_primeira_proposta_concluida",
        "data_aprovacao", "data_conclusao", "data_comite", "data_reserva", "data_entrada", "data_primeira_operacao_apos_aprovacao",
        "dias_proposta_comercial", "dias_proposta_credito", "dias_proposta_formalizacao", "tempo_conclusao", "tempo_analise"
    ]
    ...
    df_dim_clientes = df_final \
        .withColumn("desconsiderar_pdd", lit(False)) \
        .withColumn("status_risco", when(col("has_critico") == 1, "CRÍTICO").when(col("has_atencao") == 1, "ATENÇÃO").otherwise("NO PRAZO")) \
        .drop("limite_contrato", "limite_grupo_manual", "limite_extra_grupo", "limite_plus_grupo", "limite_comissaria_contrato", "limite_disponivel", "limite_maximo_disponivel", "disponivel_comissaria", "percentual_cm")

    return df_dim_clientes, df_score_clientes, df_analise_prazos

def calculate_hhi_metrics(spark, df_fato_titulos_final):
    """
    Calculates HHI metrics for the portfolio.
    """
    df_carteira_ativa = df_fato_titulos_final.filter(
    ...
    return df_hhi.withColumn("interpretacao",
